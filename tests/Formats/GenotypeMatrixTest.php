...
class GenotypeMatrixTest extends TripalTestCase {
  // Uncomment to auto start and rollback db transactions per test method.
  // use DBTransaction;

  /**
   * Tests the Genotype Matrix Format with no filtering.
   * @group formats
   * @group genotype-matrix
   */
  public function testGenotypeMatrix() {
    ...
    // -- Run the conversion.
    vcf_filter_convert_VCF_to_GenotypeMatrix(
      $input_file,
      $output_file
    );

    // 2) Ensure the output file matches what we expected.
    $expected_file = DRUPAL_ROOT . '/' . drupal_get_path('module', 'vcf_filter') . '/tests/test_files/short.genotype_matrix.txt';
